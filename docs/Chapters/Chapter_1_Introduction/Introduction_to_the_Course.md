...
This is a course in cross platform mobile software development using Xamarin.Forms and Microsoft Visual Studio 2019. The focus is on cross platform software development for mobile phones (as opposed to tablet or Desktop computers). However, with further study, other targets can easily be accommodated.
...
- Xamarin.Forms is actually three projects rolled into one:
   ...
   Google Flutter is relatively new, and takes a slightly different approach. Like Xamarin.Forms, UI is specified declaratively and not with a UI designer. However, the controls are drawn to _look like_ native controls on each respective platform. In essence, it mimic's the native UI of each platform.
...
- There is no such thing as an explanation that is too simple. Equally there are no silly questions (only silly answers ;)
...
Loading the short term memory can also result in _cognitive overload_ . Where possible, effort is made to keep all relevant information in the near field of view. Where not possible, use a diagram to show the bigger picture (e.g. relationship between objects).

---

[Next - Pre-requisite software](Pre-requisite-software.md)
