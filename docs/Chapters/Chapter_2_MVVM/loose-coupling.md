...
A class can also inherit diretly from one (and only one) parent class. In doing this, it also _inherits_ some of those (non private) behavious as well. _It is assumed you already know about class inheritance, although it wll be briefly covered below_.

Some classes are purely intended to be inherited and never directly instantiated. To achieve this, you can make a class _abstract_. Such a class is incomplete, cannot be instantiated and can only be used as a parent class. Typically, some default behavior is included, and the child classes implement the missing behaviour and override any default behaviour as appropriate.

Unlike an abstract class, an interface cannot contain any concrete code, athough new in C# v8 [some default code can now be added](https://dle.plymouth.ac.uk/course/view.php?id=45357#section-3). 
...
- All the animals breath in a slightly different ways, so having a default behaviour was deemed meaningless in this specific case
...
This is a useful feature in terms of safety. Making a base class method abstact enforces that all abstract methods are overridden by their respective subclassess. To rely on the developer to _remember_ to implement all required methods is possibly asking for trouble (and you can trust the compiler to remind you!)
...
- In the physical world, we recognise it as an animal of course.
...
So if we create a list containing type `Animal`, we can add _any subclass_ to the collection in the knowledge that it's still an `Animal`, guaranteed to implement all the behaviours of the `Animal` class.
...
Polymorphism is very useful when you don't know which concrete type will be used at runtime, but at the same time, you wish to avoid the embarrasment of a runtime crash.

### Multiple Inheritance
So far, we've only considered a single parent class `Animal` and a set of subclasses (`Caterpillar`, `Cheetah`, `Gazelle`, `GreenSeaTurtle` and `Shark`). Within these species however, there are some sub-groups. For example, the shark and the cheetah are both preditors, the tutle and shark both swim in water etc. Therefore these objects may share common behaviours which if defined in a type-safe way, could be relied upon if needed.

Like many languages, C# can only directly inherit from one parent class. Although C# does not support the full-multiple inheritance found in C++ (and all the complexities that come with it), it does allow you to inherit any number of _interfaces_. This can seem confusing at first, but it's all comes down to type-safety and enforcing guaranteed behaviour _at compile time_. 
...
To illustrate, continuing with the animal example, we can create a number of interfaces that define specific beviours. These are:

- `ICanSwim` - list methods and properties common to all swimming animals
- `ICatchPrey` - lists methods and properties common to all preditors
- `IHaveLegs` - lists methods and properties common to all animals (land or sea) with legs
- `IEatVegetation` - lists methods and properties common to all herbiores or omnivores
...
**Any** class or structure that implements this interface **must** have the property `PreditorIndex` (type `uint`) and the public method `SeekAndChasePrey()`. If they don't, the code will not compile.
...
- `ICatchPrey` because the code will not compile unless it implements `SeekAndChasePrey` and the property `preditorIndex`
...
Remember that objects can have many types. If the animal is also of type `ICatchPrey` (by implementing the interface), then it is assigned to `p` (which is type `ICatchPrey`) and the conditional statement returns `true`, in which case it can then be added to the list of preditors.
...
### Loose Coupling
For the next example, you need to look at the two other projecs in the example solution

- `MyLibrary` which is a simple .NET Core library written in C#
- `ConsoleApplication` which is a simple application which references and utilises the code contained in the library `MyLibrary`

The key points to note are as follows:

- The application instantiates concrete classes contained within the library and calls methods on those objects. This depends on knowing the class/structrure types. _The application cannot build without the library, so we say it **depends** on the library_. 
...
- The library is intended to be reused by multiple projects. Therefore, it should not depend on any concrete class/struct types in the consuming application (or any other project type that utilises it). 
   - The library can be built and distributred indepedently
...
In the simplest cases, the application instantiates types in the library and invokes methods on them. It can do this becuase it knows about the classes and structures in the library.
...
The library itself does not implmenent this interface. It is expected that the application consuming the library will implement the interface. Let's first look at the library code:
...
- It is the appliction that will implement the interface, so it is the application that guarantees implementation the methods `LogMessage` and `Complete`
...
Another approach might have been to pass a closure as a parameter. This also works, but where there are several methods that need to be invoked, the code might get inweildy.
